  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-semibold font-display text-gray-900">General Site FAQs</h1>
        {!isAdding && !editingId && (
          <button 
            onClick={() => setIsAdding(true)} 
            className="bg-brand text-white px-4 py-2 rounded-xl font-bold flex items-center gap-2 hover:bg-brand/90"
          >
        ...
        <div className="bg-white border border-gray-200 p-6 rounded-2xl mb-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex-1 flex items-center mt-6">
                <label className="flex items-center gap-2 cursor-pointer font-bold">
                  <input 
                    ...
                    className="w-5 h-5 rounded text-brand border-gray-200 focus:ring-primary"
                  />
                  Is Active
                </label>
              </div>
            ...
            <div className="flex gap-2 pt-4">
              <button type="submit" className="bg-brand text-white px-6 py-2 rounded-xl font-bold flex items-center gap-2 hover:bg-brand/90">
                <Check className="w-4 h-4" /> Save
              </button>
              <button type="button" onClick={cancelEdit} className="bg-gray-100 text-gray-900 px-6 py-2 rounded-xl font-bold flex items-center gap-2 hover:bg-gray-200">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-border">
            {faqs.map(faq => (
              <tr key={faq.id} className="hover:bg-kraft/5">
                ...
                <td className="px-6 py-4">
                  ...
                  <div className="text-sm text-gray-500 truncate max-w-xl">{faq.answer}</div>
                </td>
                <td className="px-6 py-4">
            ...
            {faqs.length === 0 && (
              <tr>
                <td colSpan="4" className="px-6 py-8 text-center text-gray-500">No FAQs found. Add one above.</td>
              </tr>
            )}
          </tbody>
        </table>
